...
def allTreeSearch(bitboard,returnAction=False):
    if bitboard.isFinished():
        return bitboard.board[bitboard.turn]>bitboard.board[1-bitboard.turn]
    elif bitboard.isPass():
        bitboard.turn=1-bitboard.turn
        return not allTreeSearch(bitboard)
    legalBoard=bitboard.makeLegalBoard()
    mask=0x8000000000000000
    way=-1
    for i in range(64):
        if legalBoard & mask:
            if not allTreeSearch(bitboard.choose_pos_int(i%8,i//8)):
                if returnAction:
                    return i
                else:
                    return True
...
def solve(bitboard):
    if bitcount(bitboard.board[0]|bitboard.board[1])>=54:
        choose=allTreeSearch(bitboard,True)
    else:
...
while True:
    ...
    for i in range(action_count):
        action = input()  
    print(solve(bb),'MSG',stable_stone(bb,bb.turn))